...
import { PlayerMaps, WebScraper } from "../WebScraper/WebScraper";
...
export const stats: CommandCallback = async (message, command, _) => {
  let args = parseArgs(_.join(" "));
  let flags = args.map((x) => x.flag);
  if (!args.find((x) => x.flag == "default"))
    return message.channel.send(
      `Invalid Command Usage; expected: \`-stats <player name>\``
    );
  ...
  if (flags.includes("-map") || flags.includes("-m")) {
    ...
    if (flags.includes("-difficulty") || flags.includes("-d")) {
      let difficulty = args.find(
        (x) => x.flag == "-difficulty" || x.flag == "-d"
      ).arg;
      ...
      didYouMean.push(
        ...maps
          ...
          .maps.filter((x) =>
            x.name.toLowerCase().startsWith(mapName.toLowerCase())
          )
          .map((x) => x.name)
      );
    } else {
      for (let i = 0; i < maps.length; i++) {
        let x = maps[i].maps.find(
          (x) => x.name.toLowerCase() == mapName.toLowerCase()
        );
        if (x) {
          map = x;
          break;
    ...
    if (!map)
      return message.channel.send({
        content: `I could not find that map, are you sure you spelt it correctly. ${
          didYouMean.length ? `\n\nDid You Mean:\n${didYouMean.join("\n")}` : ""
        }`,
      });
